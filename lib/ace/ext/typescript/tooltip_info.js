define(function (require, exports, module) {
    ...
    var TooltipInfo = function (editor, languageService, tempFileName) {
        ...
        event.addListener(editor, "change", this.onChange);
    };
    ...
    (function () {
        ...
        function getTooltipText(typeInfo, sigInfo) {
            ...
            if (typeInfo.memberName) {
                if (typeInfo.memberName.prefix) {
                    return typeInfo.memberName.prefix;
                }
                if (typeInfo.memberName.entries) {
                    return typeInfo.memberName.entries.toString();
                }
            }
        }
        ...
        this.update = function () {
            ...
            var sigInfo = languageService.getSignatureAtPosition(this.tempFileName, typeInfo.limChar);
        ...
        this.requestMethodInfoAtCursor = function () {
            if (this.$timer) clearTimeout(this.$timer);
            this.$timer = setTimeout(this.displayMethodInfoAtCursor.bind(this), 100);
        }

        this.displayMethodInfoAtCursor = function () {
            var self = this;
            var cursor = this.editor.getCursorPosition();
            var pos = this.editorPos.getPositionChars(cursor);
            var signatureInfo = this.languageService.getSignatureAtPosition(this.tempFileName, pos);

            if (!signatureInfo) return;

            var tooltipText = signatureInfo.formal.map(function (item) {
                return item.signatureInfo;
            }).join('\r\n');
            ...
            top = coords.pageY - 10 - self.tooltipHeight;
            var clientRect = this.editor.container.getBoundingClientRect();
            self.y = top + 'px';
            self.x = coords.pageX + 'px';
            self.updateTooltipPosition(self.x, self.y);
        }
        ...
        this.destroy = function () {
            ...
            event.removeListener(this.editor, "change", this.onChange);
