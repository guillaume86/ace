...
(function() {
    ...
    this.registerCommands = function(handler) {
        ...
        var commands = [{
            ...
            name: "tooltipInfoMethod",
            bindKey: "(",
            exec: function () {
                self.editor.commands.exec("insertstring", self.editor, '(');
                self.requestTooltipInfo();
            }
        }];
    ...
    this.refactor = function() {
        ...
        references.forEach(function (ref) {
            var start = editorPosition.getAcePositionFromChars(ref.minChar);
            var end = editorPosition.getAcePositionFromChars(ref.limChar);
            var range = new AceRange(start.row, start.column, end.row, end.column);
    ...
    this.goToDefinition = function() {
        var editor = this.editor;
        var editorPosition = this.editorPosition;
        var defs = this.languageService.getDefinitionAtPosition(this.tempFileName, editorPosition.getCurrentCharPosition());
        var session = editor.getSession();
        ...
        var start = getpos(def.minChar);
        var end = getpos(def.limChar);
        var range = new AceRange(start.row, start.column, end.row, end.column);
    ...
    this.requestTooltipInfo = function () {
        var tooltipInfo = this.tooltipInfo;
        tooltipInfo.requestMethodInfoAtCursor();
    };
